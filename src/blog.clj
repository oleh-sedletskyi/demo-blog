...
(defn render-post-html [{:keys [metadata] :as post}]
  (let [template (slurp "templates/post.html")
        updated (-> template
                    (str/replace #"CONTENT-TO-REPLACE" (:html post))
                    (str/replace #"DATE-TO-REPLACE" (->> (:date metadata)
                                                         t/date
...
(defn render-index-html [main-html]
  (let [template (slurp "templates/index.html")
...
(defn render-main-pages []
  (let [main-files (->> (file/list-dir-files "content")
                        (filter #(some (partial str/includes? %) ["about"])))]
    (when (seq main-files)
...
#_(defn add-keywords-description! [{:keys [metadata markdown]}]
...
(defn ->md
  ...
  (let [frontmatter (str "---\n"
                         (yaml/generate-string
                          ...
                          :dumper-options {:flow-style :block})
                         "---\n\n")]
    (spit (str "content/posts/" alias ".md") (str frontmatter text))))
...
(defn process-posts
  ...
  (let [posts-path "content/posts/"
        ...
        parsed-files (->> files
                          (map parse-md-file)
                          (sort-by (fn [m] (-> (get-in m [:metadata :date])
                                               (t/date)))
                                   #(compare %2 %1)))
    ...
    (-> (h/html
         [:main {:class "container"}
          (->> metadata
               (map (fn [{:keys [introtext date title html-path]}]
                      [:section
                       [:hgroup
                        [:h2
